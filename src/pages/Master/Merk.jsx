...
import { HiPlus, HiPencil, HiTrash, HiSearch, HiBriefcase, HiChevronLeft, HiChevronRight, HiX } from 'react-icons/hi'
        ...
        <table className="w-full whitespace-no-wrap">
          <thead>
            <tr className="text-xs font-bold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-900/50">
              ...
              <th className="px-6 py-4">Kode Merk</th>
              ...
              data.map((row, index) => (
                <tr key={row.ID || index} className="text-gray-700 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <span className="font-bold text-gray-800 dark:text-gray-200">{row.nama_merk}</span>
                  </td>
...
function Merk() {
  ...
  const openModal = (item = null) => {
    if (item) {
      ...
      setValue('nama_merk', item.nama_merk)
    } else {
      setEditingId(null)
      reset({ nama_merk: '' })
    }
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 flex items-center gap-3">
              <div className="p-2 bg-purple-100 dark:bg-purple-900/30 rounded-xl">
                <HiBriefcase className="text-purple-600 w-6 h-6" />
              </div>
              Master Merk
            </h2>
            <p className="text-xs text-gray-500 mt-1 ml-11">Kelola daftar merk atau brand barang inventaris.</p>
          </div>
              ...
              <form onSubmit={handleSubmit(onFormSubmit)}>
                <div className="p-8">
                  <label className="block text-sm">
                    <span className="text-gray-700 dark:text-gray-400 font-bold uppercase text-[10px]">Nama Merk</span>
                    <input
                      {...register('nama_merk', { required: true })}
