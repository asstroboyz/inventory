...
import { HiPlus, HiPencil, HiTrash, HiSearch, HiRefresh, HiChevronLeft, HiChevronRight, HiX } from 'react-icons/hi'
        ...
        <table className="w-full whitespace-no-wrap">
          <thead>
            <tr className="text-xs font-bold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-900/50">
              ...
              <th className="px-6 py-4">Nama Jenis Transaksi</th>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan="4" className="px-6 py-20 text-center text-gray-500">
                  Tidak ada data jenis transaksi.
                </td>
              </tr>
            ) : (
              data.map((row, index) => (
...
function JenisTransaksi() {
  ...
  const fetchData = useCallback(async (searchVal = searchTerm) => {
    ...
    setLoading(true)
    try {
      const { data } = await axios.post(`${BaseUrl}/api/master/jenis-transaksi/cari`, {
  ...
  const openModal = (item = null) => {
    if (item) {
      ...
      setValue('kode_jenis', item.kode_jenis)
    } else {
      setEditingId(null)
      reset({ nama_jenis: '', kode_jenis: '' })
    }
    setIsModalOpen(true)
  }
  ...
  const onFormSubmit = async (formData) => {
    try {
      const url = `${BaseUrl}/api/master/jenis-transaksi/`
      ...
      if (res.status === 200 || res.status === 201) {
        toast.success(editingId ? 'Jenis transaksi diperbarui!' : 'Jenis transaksi ditambah!')
        closeModal()
        fetchData()
      } else {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Yakin ingin menghapus jenis transaksi ini?')) return
    try {
      const res = await axios.delete(`${BaseUrl}/api/master/jenis-transaksi/${id}`, config)
      if (res.status === 200) {
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 flex items-center gap-3">
              <div className="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-xl">
                <HiRefresh className="text-blue-600 w-6 h-6" />
              </div>
              Master Jenis Transaksi
            </h2>
            <p className="text-xs text-gray-500 mt-1 ml-11">Kelola kategori transaksi (Masuk, Keluar, Mutasi, dll).</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3">
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-2xl dark:text-gray-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 outline-none transition-all shadow-sm"
                value={searchTerm}
                onChange={(e) => {
            ...
            <button
              onClick={() => openModal()}
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-2xl hover:bg-blue-700 transition-all shadow-lg active:scale-95 whitespace-nowrap"
            >
            ...
            <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-3xl shadow-2xl overflow-hidden border border-white/10">
              <header className="px-8 py-6 border-b dark:border-gray-700 flex justify-between items-center">
                <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200">
                  {editingId ? 'Edit Jenis Transaksi' : 'Tambah Jenis Transaksi Baru'}
                </h3>
              ...
              <form onSubmit={handleSubmit(onFormSubmit)}>
                <div className="p-8 space-y-6">
                  <label className="block text-sm">
                    <span className="text-gray-700 dark:text-gray-400 font-bold uppercase text-[10px]">Nama Jenis Transaksi</span>
                    <input
                      {...register('nama_jenis', { required: true })}
                      placeholder="Contoh: Barang Masuk, Mutasi"
                      className="form-input mt-2"
                    />
                  </label>
                  <label className="block text-sm">
                    <span className="text-gray-700 dark:text-gray-400 font-bold uppercase text-[10px]">Kode Jenis (Opsional)</span>
                    <input
                      {...register('kode_jenis')}
