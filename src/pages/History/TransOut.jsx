...
const TransOut = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch(`${BaseUrl}/api/record/history/out/`, {
      ...
      <div className="container px-6 mx-auto grid pb-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between my-8 gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1 ml-11 italic">Log riwayat pengeluaran atau mutasi barang keluar.</p>
          </div>
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
                {loading ? (
                  ...
                  paginatedData.map((row, index) => (
                    <tr key={row.ID || index} className="text-gray-700 dark:text-gray-400 hover:bg-red-50/30 dark:hover:bg-red-900/10 transition-colors group">
